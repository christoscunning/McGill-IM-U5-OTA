...
/**
 * @brief   Receives a <dataLength> number of bytes through the specified UART interface in polling mode.
 * @param   huart The handle of the UART that will be used to receive the data.
 * @param   dataLength The number of bytes that will be received. This function will block until <dataLength> number
...
int uart_rx(UART_HandleTypeDef *huart, int dataLength, char *data) {
  ...
  printf("uart_rx() - dataLength: %d\r\n", dataLength);
	...
	while (data_to_read) {
		//ret = ReadFile(uart_handle, data, data_to_read, &data_read, NULL);
...
/**
 * @brief   Mimics the behavior of uart_rx() function, but utilizing the UART interrupt buffers. Non-blocking function.
 *
